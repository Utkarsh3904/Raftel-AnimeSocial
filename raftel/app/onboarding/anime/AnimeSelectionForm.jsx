...
export default function AnimeSelectionForm() {
  ...
  const handleFinish = async () => {
    ...
    try {
      const res = await fetch("/api/users/me", {
        ...
        body: JSON.stringify({ top5Anime: selectedAnime }),
      })
      ...
      if (res.ok) {
        ...
        toast.error(data.error || "Failed to save anime")
      }
    } catch {
        ...
        {selectedAnime.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {selectedAnime.map((anime) => (
              ...
              >
                <img src={anime.coverImage.medium} className="w-4 h-4 rounded-full object-cover" alt="" />
                {anime.title.romaji}
